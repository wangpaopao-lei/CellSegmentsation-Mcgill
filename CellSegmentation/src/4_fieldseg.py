...
def xy2gene(x, y, exp,start_x,start_y):
    def xy2idx(x, y):
        return int(math.floor(int(x)+start_x) * math.ceil(1200) + math.floor(int(y)+start_y))
    
    return exp[xy2idx(x, y)].toarray().ravel()
...
if __name__ == "__main__":
    ...
    for i, (x, y, prob, cell) in enumerate(nuclei):
        ...
        if line_count % 10000 == 0:
            ...
            print(
                f"{int(line_count/10000)} / 144 batches completed! Time for batch: {elapsed_time:.2f} seconds"
            )
