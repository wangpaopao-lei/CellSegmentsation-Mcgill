        ...
        for line in fr:
            ...
            for center in watershed2center:
                ...
                if dis_c <= int(dia_estimate):
                    class_prior += dir_to_class(watershed2center[center][0] - int(x),
                                                watershed2center[center][1] - int(y), class_num,
                                                watershed2center[center][0], watershed2center[center][1])
                if dis_c <= int(dia_estimate) * 1.25:
                    class_prior_ext += dir_to_class(watershed2center[center][0] - int(x),
                                                    watershed2center[center][1] - int(y), class_num,
                                                    watershed2center[center][0], watershed2center[center][1])
    ...
    for index, (x, y) in enumerate(zip(i, j)):
        ...
        while (cosphi > 0):
            ...
            if Mapped[int(Trajectory[points, 0]), int(Trajectory[points, 1])] == 1:
                ...
                cosphi = dy[int(Trajectory[points - 1, 0]), int(Trajectory[points - 1, 1])] * dy[
                    int(Trajectory[points, 0]), int(Trajectory[points, 1])] + dx[
                             int(Trajectory[points - 1, 0]), int(Trajectory[points - 1, 1])] * dx[
                             int(Trajectory[points, 0]), int(Trajectory[points, 1])]
    ...
    for i in range(label_mat.shape[0]):
        for j in range(label_mat.shape[1]):
            if i % downrs == 0 and j % downrs == 0 and label_mat[i, j] == 0:
                neighbor_labels = []
                for d in [[-downrs, -downrs], [-downrs, 0], [-downrs, downrs], [downrs, -downrs], [downrs, 0],
                          [downrs, downrs], [0, -downrs], [0, downrs]]:
                    if i + d[0] >= 0 and i + d[0] < label_mat.shape[0] and j + d[1] >= 0 and j + d[1] < label_mat.shape[
                        1]:
...
def postprocess(startx, starty, patchsize, bin_size, dia_estimate):
    ...
    intensity, dx, dy, pred_U, pred_V, pred_C = read_gradient(
        'results/spot_prediction_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', adata,
        (int(patchsizex), int(patchsizey)), bin_size, dia_estimate)
