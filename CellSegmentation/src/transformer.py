...
def create_transformer_classifier(class_num, input_shape, input_position_shape, num_patches, projection_dim, num_heads,
...
def run_experiment(startx, starty, patchsize, model, x_train, x_train_pos, x_train_, x_train_pos_, y_train, y_train_,
                   y_binary_train, x_test, x_test_pos, x_validation, x_validation_pos, y_validation,
                   y_binary_validation, learning_rate, weight_decay, batch_size, num_epochs):
    ...
    model.fit(
        ...
        validation_data=(
        [x_validation[np.where(y_binary_validation == 1)], x_validation_pos[np.where(y_binary_validation == 1)]],
        [y_validation[np.where(y_binary_validation == 1)], y_binary_validation[np.where(y_binary_validation == 1)]]),
    ...
    for i in range(int(len(x_test) / 10000) + 1):
        pred_centers_test_, pred_binary_test_ = model.predict(
            x=[x_test[i * 10000: (i + 1) * 10000], x_test_pos[i * 10000: (i + 1) * 10000]], batch_size=batch_size)
    ...
    for i in range(int(len(x_train_) / 10000) + 1):
        pred_centers_train_, pred_binary_train_ = model.predict(
            x=[x_train_[i * 10000: (i + 1) * 10000], x_train_pos_[i * 10000: (i + 1) * 10000]], batch_size=batch_size)
    ...
    with open('results/spot_prediction_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt',
              'w') as fw:
        for i in range(len(y_train_)):
            fw.write(str(x_train_pos__[i][0][0]) + '\t' + str(x_train_pos__[i][0][1]) + '\t' + str(
                pred_binary_train[i][0]) + '\t' + ':'.join([str(c) for c in pred_centers_train[i]]) + '\n')
        for i in range(len(x_test_pos_)):
            fw.write(str(x_test_pos_[i][0][0]) + '\t' + str(x_test_pos_[i][0][1]) + '\t' + str(
                pred_binary_test[i][0]) + '\t' + ':'.join([str(c) for c in pred_centers_test[i]]) + '\n')
...
def train(startx, starty, patchsize, epochs, val_ratio):
    ...
    y_binary_train_ = np.load(
        'data/y_binary_train_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz')
    ...
    for i in range(len(x_train_pos_)):
        if x_train_pos_[i][0][0] > int(adata.X.shape[0] * (1 - np.sqrt(val_ratio))) and x_train_pos_[i][0][1] > int(
    ...
    transformer_classifier = create_transformer_classifier(class_num, input_shape, input_position_shape, num_patches,
                                                           projection_dim, num_heads, transformer_units,
                                                           transformer_layers, mlp_head_units)
    run_experiment(startx, starty, patchsize, transformer_classifier, x_train, x_train_pos, x_train_, x_train_pos_,
                   y_train, y_train_, y_binary_train, x_test, x_test_pos, x_validation, x_validation_pos, y_validation,
                   y_binary_validation, learning_rate, weight_decay, batch_size, num_epochs)
