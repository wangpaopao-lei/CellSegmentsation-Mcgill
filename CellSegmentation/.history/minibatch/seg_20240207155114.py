...
if __name__=="__main__":
    ...
    for i, (x, y, prob, cell) in enumerate(task2_result):
        if cell != 0:
            continue
        # 使用NumPy数组来执行元素级的减法
        initial_field_vectors = np.array([np.array(centroids[c][:2]) - np.array([x, y]) for c in centroids])
        distances = np.linalg.norm(initial_field_vectors, axis=1)

        # 过滤距离超过max_radius的细胞核
        valid_indices = distances < max_radius
        valid_field_vectors = initial_field_vectors[valid_indices]
        valid_centroids = [centroids[c] for c, dist in zip(centroids, valid_indices) if dist]
        ...
        # 初始化列表用于累积场力向量
        accumulated_field_vectors = []  # 使用新的变量名来累积场力向量

        # 计算场力向量
        for c, vec, dist in zip(valid_centroids, valid_field_vectors, distances[valid_indices]):
            vec_normalized = vec / np.linalg.norm(vec)  # 规范化vec
            weight_factor = 1 / dist**2
            similarity_measure = similarity_expression_vectors(c, x, y, R)
            
            print(similarity_measure)
            
            weighted_vector = weight_factor * similarity_measure * vec
            accumulated_field_vectors.append(weighted_vector)  # 向列表中添加元素

        # 将累积向量的列表转换为NumPy数组以便后续处理
