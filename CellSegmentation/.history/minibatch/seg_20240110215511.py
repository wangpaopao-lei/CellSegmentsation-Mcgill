...
for cell in unique_cells:
    ...
    x, y = task2_result[mask, :2].T
    x, y = x.astype(int), y.astype(int)
    segmentation_image[y, x] = colors[cell_index][:3] * 255  # 确保索引是整数
...
def similarity_expression_vectors(cell, x, y, R):
    ...
    # 计算皮尔逊相关系数
    correlation = np.corrcoef(cell_expression_noisy, point_expression_noisy)[0, 1]
