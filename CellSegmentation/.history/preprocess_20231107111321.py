...
dataset_map = np.ones_like(label_img,dtype=float)*255
train_color = [255, 0, 0]  # 红色
test_color = [0, 0, 255]   # 蓝色
...
x_optimize_train_outer = []
x_optimize_pos_outer = []
x_optimize_labels_outer = []
y_optimize_train_outer = []
y_bin_outer = []
...
for i in range(adatasub.layers['watershed_labels'].shape[0]):
    ...
    for j in range(adatasub.layers['watershed_labels'].shape[1]):
        ...
        # For Optimize_train
        if label > 0 and distance >= distance_ratio_threshold and np.sum(all_exp_merged_bins[idx, :]) > 0:
            counter_core += 1
            ...
            x_optimize_labels.append(np.array(x_optimize_labels_sample))
            y_optimize_train.append(np.array(y_optimize_sample))
            y_bin.append(1)
        # For x_test
        elif label == 0:
            counter_outer += 1
            ...
            x_optimize_train_outer.append(x_optimize_sample)
            x_optimize_pos_outer.append(x_optimize_pos_sample)
            x_optimize_labels_outer.append(np.array(x_optimize_labels_sample))
            y_optimize_train_outer.append(np.array([-1,-1]))
...
for i in range(adatasub.layers['watershed_labels'].shape[0]):
    ...
    for j in range(adatasub.layers['watershed_labels'].shape[1]):
        ...
        # 忽略已经处理过的核心点和核外点
        if label > 0 and (distance < distance_ratio_threshold or np.sum(all_exp_merged_bins[idx, :]) == 0):
            continue
        if label == 0 and np.sum(all_exp_merged_bins[idx, :]) == 0:
            continue
...
plt.imsave('/fi/dataset_map.png', dataset_map)

num_samples = min(len(x_optimize_train), len(x_optimize_train_outer))
x_optimize_train = x_optimize_train + x_optimize_train_outer[:num_samples]
x_optimize_pos=x_optimize_pos+x_optimize_pos_outer[:num_samples]
x_optimize_labels=x_optimize_labels+x_optimize_labels_outer[:num_samples]
y_optimize_train=y_optimize_train+y_optimize_train_outer[:num_samples]
y_bin=y_bin+y_bin_outer[:num_samples]
...
print(counter_core)
print(counter_outer)
print(x_optimize_pos.shape)
print(x_optimize_labels.shape)
print(x_optimize_train.shape)
print(y_optimize_train.shape)
print(x_test.shape)
