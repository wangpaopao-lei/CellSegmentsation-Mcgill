...
def load_batch(file_path, start_idx, batch_size):
    ...
    x_test_pos = data['x_test_pos'][start_idx:start_idx+batch_size].astype(np.int32)
...
def pred(model,batch_size=10):
    ...
    for i in range(int(total_size / pred_batch_size) + 1):
        ...
        print(f"Batch {i} completed. Time taken: {elapsed_time:.2f} seconds", flush=True)
