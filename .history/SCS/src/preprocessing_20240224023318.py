...
def preprocess(bin_file, image_file, prealigned, align, startx, starty, patchsize, bin_size, n_neighbor):
    ...
    st.pl.imshow(adatasub, 'unspliced', ax=axes[0], alpha=0.6, cmap='Reds', vmax=10, use_scale=False,
                 save_show_or_return='return')
    axes[0].set_title('before alignment')
    st.pl.imshow(adatasub, 'stain', ax=axes[1], use_scale=False, save_show_or_return='return')
    st.pl.imshow(adatasub, 'unspliced', ax=axes[1], alpha=0.6, cmap='Reds', vmax=10, use_scale=False,
                 save_show_or_return='return')
    ...
    np.save("all_exp_merged_bins.npy", all_exp_merged_bins)
    # print(all_exp_merged_bins.shape)
    cell_bins = {}
    x_train_tmp = []
    x_train = []
    ...
    for i in range(adatasub.layers['watershed_labels'].shape[0]):
        ...
        for j in range(adatasub.layers['watershed_labels'].shape[1]):
            ...
            if adatasub.layers['watershed_labels'][i, j] > 0:
                if 0 <= idx < all_exp_merged_bins.shape[0] and np.sum(all_exp_merged_bins[idx, :]) > 0:
                ...
                if 0 <= idx < all_exp_merged_bins.shape[0] and np.sum(all_exp_merged_bins[idx, :]) > 0:
                    ...
                    if backgroud:
                        ...
                        for dx, dy in offsets:
                            ...
                            if 0 <= idx_nb < all_exp_merged_bins.shape[0] and np.sum(
                                    all_exp_merged_bins[idx_nb, :]) > 0:
                                x_train_sample.append(all_exp_merged_bins[idx_nb, :])
                                x_train_pos_sample.append([x, y])
                        if len(x_train_sample) < n_neighbor:
    ...
    np.savez_compressed('data/x_train_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz',
                        x_train=x_train)
    np.savez_compressed('data/x_train_pos_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz',
                        x_train_pos=x_train_pos)
    np.savez_compressed('data/y_train_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz',
                        y_train=y_train)
    np.savez_compressed('data/y_binary_train_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz',
                        y_binary_train=y_binary_train)
    np.savez_compressed('data/x_test_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz',
                        x_test=x_test)
    np.savez_compressed('data/x_test_pos_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.npz',
