...
def create_matrices(
    ...
    size (Tuple[int, int], optional): The size of the matrices. Defaults to (1200, 1200).
    ...
    task1_matrix[task1_result[:, 0].astype(int), task1_result[:, 1].astype(int)] = (
        task1_result[:, 2]
    )
    watershed_matrix[watershed[:, 0].astype(int), watershed[:, 1].astype(int)] = (
        watershed[:, 2]
    )
