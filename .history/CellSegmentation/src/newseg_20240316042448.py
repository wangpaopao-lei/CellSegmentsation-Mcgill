...
def xy2gene(x: np.ndarray, y: np.ndarray, exp, start_x: int, start_y: int) -> np.ndarray:
    """
    Convert x, y coordinates to gene expression data using a sparse matrix.

    Args:
    x (np.ndarray): x coordinates.
    y (np.ndarray): y coordinates.
    exp (scipy.sparse.csr_matrix): Expression data in a sparse matrix format.
    start_x (int): Starting x offset.
    start_y (int): Starting y offset.

    Returns:
    np.ndarray: Gene expression data for the given coordinates.
    """
    idx = int(math.floor(int(x)+start_x) * math.ceil(1200) + math.floor(int(y)+start_y))
    return exp[idx].toarray().ravel()
...
def similarity_expression_vectors(cell, exp, x, y, R, nucleus_expression,start_x,start_y):

    point_expression = xy2gene(x, y, exp,start_x,start_y)
...
if __name__ == "__main__":
    ...
    nuclei_path="results/task2_result.txt"
    expression_csr_matrix = load_npz(expression_path)
    nuclei=np.loadtxt(nuclei_path)
    ...
    # 创建  cell 总基因表达的空字典
    nucleus_expression = {
        cell: np.zeros(expression_csr_matrix.shape[1]) for cell in centroids if cell != 0
    }
    # 创建 cell 像素个数的空字典
    ...
    for cell in unique_cells:
        ...
        for xi, yi in zip(x, y):
            nucleus_expression[cell] += xy2gene(xi, yi, expression_csr_matrix,start_x,start_y)
            nucleus_pixel_count[cell] += 1

    print("executing field segmentation\n")
    
    background_threshold = 0  # 概率小于此值的点视为背景点
    cell_threshold = 0.0005  # 合力大于此值的点视为细胞点
    R = 1
    max_radius = 28  # 设置最大的细胞半径
    total_line=nuclei.shape[0]
    ...
    # TODO: 把nuclei读取方式改为 loadtxt，使用 ndarray
    for i, (x, y, prob, cell) in enumerate(nuclei):
        ...
        if line_count % 10000 == 0:
            ...
            print(
                f"{int(line_count/10000)} / {int(total_line/10000)} batches completed! Time for batch: {elapsed_time:.2f} seconds"
            )
        ...
        for c, vec, dist in zip(
            ...
            similarity_measure = similarity_expression_vectors(
                c, expression_csr_matrix, x, y, R, nucleus_expression,start_x,start_y
            )
    ...
    image_width = np.max(nuclei[:, 0]).astype(int) + 1
    image_height = np.max(nuclei[:, 1]).astype(int) + 1
