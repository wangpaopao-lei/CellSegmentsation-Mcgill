...
def segment_nucleus(adatasub):
    ...
    """
    cs.mask_nuclei_from_stain(adatasub, otsu_classes=4, otsu_index=1)
    cs.find_peaks_from_mask(adatasub, "stain", 7)
    cs.watershed(adatasub, "stain", 5, out_layer="watershed_labels")

    fig, ax = plt.subplots(figsize=(8, 8), tight_layout=True)
    pl.imshow(adatasub, "watershed_labels", labels=True, alpha=0.5, ax=ax)
...
def preproccess_gene_data(
    ...
    save_npz("dataset/full_gene_sparse_matrix.npz", all_exp_merged_bins)
    ...
    all_exp_merged_bins_ad.write("dataset/full_gene.h5ad")
    print("h5ad saved!")
...
def prepare_samples(
    layers, shape, expression_matrix, offsets, threshold, n_neighbor, downrs
):
    """准备训练和测试样本。"""
    x_train, x_train_pos, x_train_labels, y_train = [], [], [], []
    x_train_outer, x_train_pos_outer, x_train_labels_outer, y_train_outer = (
        [],
        [],
        [],
        [],
    ...
    temp_dir = "dataset/x_test_batches"
    try:
        os.mkdir(temp_dir)
    except FileExistsError:
        print("batch folder exists")
    ...
    for i in range(shape[0]):
        ...
        for j in range(shape[1]):
            ...
            if (
                ...
                x_train_labels.append(labels)
                y_train.append(1)
            elif label == 0 and i % downrs == 0 and j % downrs == 0:
                counter_outer += 1
                x_train_outer.append(samples)
                x_train_pos_outer.append(pos)
                x_train_labels_outer.append(labels)
                y_train_outer.append(0)

            x_test_tmp.append(samples)
            if len(x_test_tmp) >= test_batch_size:
                batch_path = os.path.join(
                    temp_dir, f"x_test_batch_{test_batch_count}.npz"
                )
                np.savez_compressed(batch_path, x_test=np.array(x_test_tmp))
                test_batch_index.append(batch_path)
    ...
    if x_test_tmp:
        ...
        np.savez_compressed(batch_path, x_test=np.array(x_test_tmp))
        test_batch_index.append(batch_path)
    index_path = os.path.join(temp_dir, "batch_index")
    with open(index_path, "w") as index_file:
        for batch in test_batch_index:
            index_file.write(batch + "\n")

    # balance samples between core and outer
    ...
    x_train_pos = x_train_pos + [x_train_pos_outer[i] for i in random_indices]
    x_train_labels = x_train_labels + [x_train_labels_outer[i] for i in random_indices]
...
if __name__ == "__main__":
    ...
    start_x = args.patch[0]
    start_y = args.patch[1]
    end_x = args.patch[2]
    end_y = args.patch[3]
