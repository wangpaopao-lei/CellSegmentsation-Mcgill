...
def load_batch(file_path, start_idx, batch_size):
    ...
    """
    x_test = np.load(file_path, allow_pickle=True)["x_test"].astype(np.float32)
    x_test_pos=np.load("dataset/x_test_pos.npz",allow_pickle=True)["x_test_pos"][start_idx : start_idx + batch_size].astype(int)
    x_test_labels = np.load("dataset/x_test_labels.npz",allow_pickle=True)["x_test_labels"][start_idx : start_idx + batch_size].astype(np.float32)
...
def run_experiment(
    ...
    with open("dataset/x_test_batches/batch_index",'r') as index_file:
        batches=index_file.readlines()
        
    print(f"{total_size/pred_batch_size} in total")
    total_size = 10000*len(batches)
    ...
    x_test_pos_ = np.load("dataset/x_test_pos.npz")
...
def train(epochs=30, val_ratio=0.2):
