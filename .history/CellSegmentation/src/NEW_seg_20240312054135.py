...
if __name__ == "__main__":
    ...
    for cell in unique_cells:
        if cell == 0:
            continue
        mask = nuclei.iloc[:, 3] == cell

        selected_data = nuclei.loc[mask, nuclei.columns[:3]].values
        x, y, prob = selected_data.T
        ...
        for xi, yi in zip(x, y):
            ...
            # TODO：需要确认这里的相加需要用什么格式
            nucleus_expression[cell] += xy2gene(xi, yi, expression_matrix)
    ...
    # TODO: 更新细胞核的基因表达的计算方式
    for i, row in nuclei.iterrows():
        x, y, prob, cell = row[0], row[1], row[2], row[3]
        line_count += 1
