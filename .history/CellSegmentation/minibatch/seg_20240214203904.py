...
def aggregate_expression(exp, x, y, patchsizey):
    # 初始化聚合后的表达向量
    aggregated_expression = np.zeros_like(exp[0, :])
    count = 0
    ...
    for dx in range(-1, 2):
        for dy in range(-1, 2):
            ...
            ny = y + dy
            # 确保邻域点在边界内
            if 0 <= nx < 800 and 0 <= ny < patchsizey:
...
if __name__=="__main__":
    ...
    for i, (x, y, prob, cell) in enumerate(task2_result):
        ...
        for c, vec, dist in zip(valid_centroids, valid_field_vectors, distances[valid_indices]):
            ...
            weight_factor=1/dist
            similarity_measure = similarity_expression_vectors(c,all_exp_merged_bins, x, y, R,nucleus_expression)
        ...
        if cell_threshold<mt1<cell_threshold+cell_threshold/3 and task2_result[i, 3]==54:
            markers.append((x,y))
