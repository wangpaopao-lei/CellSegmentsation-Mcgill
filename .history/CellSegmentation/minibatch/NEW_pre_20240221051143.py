...
def crop_image_data(adatasub, start_x, start_y, end_x, end_y):
    ...
    empty_X=np.zeros((start_y-end_y, start_x-end_x))
...
def preproccess_gene_data(bin_file,patchsizex,patchsizey, start_x, start_y, end_x, end_y):
    """
    process bin data
    ...
    # 转换为二维坐标索引的数组
    expression_matrix = np.zeros((end_y-start_y, end_x-start_x, len(selected_index)))
...
def prepare_samples(layers,shape,expression_matrix,offsets, threshold,n_neighbor):
    """准备训练和测试样本。"""
    ...
    for i in range(shape[0]):
        ...
        for j in range(shape[1]):
            ...
            label, distance = layers['watershed_labels'][i, j], layers['watershed_distance'][i, j]
            
            samples, pos, labels = process_sample(i, j, distance, offsets,layers,expression_matrix,shape,n_neighbor)
            
            if label > 0 and distance >= threshold:
                counter_core += 1
    ...
    return x_train, x_pos, x_labels,y_train,x_test,x_test_pos,x_test_labels, counter_core, counter_outer
    

def process_sample(i, j, distance, offsets, layers, expression_matrix,shape, n_neighbor):
    """
...
if __name__=="__main__":
    ...
    expression_matrix = preproccess_gene_data(bin_file,patchsizex,patchsizey,start_x,start_y,end_x,end_y)
    offsets = calculate_offsets(10)
    x_train,x_pos, x_labels,y_train,x_test,x_test_pos,x_test_labels, counter_core, counter_outer = prepare_samples(
        adatasub.layers, 
    ...
    np.savez_compressed('dataset/x_pos.npz', x_pos=x_pos)
    np.savez_compressed('dataset/x_labels.npz', x_labels=x_labels)
