...
def save_results(task1_result, final_labels, filename='results/task2_result.txt'):
    ...
    filename: 结果文件的路径。
    """
    task1_result[:, 3] = final_labels[task1_result[:, 0].astype(int), task1_result[:, 1].astype(int)]
...
def main():
    ...
    # 标记、比较和处理
    final_labels = label_and_compare(task1_matrix, watershed_matrix, threshold, min_cell_size)
